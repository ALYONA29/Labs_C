...
void show_node(struct List* list)
{
	...
	{
		printf("%d: %s %s %lld", i + 1, node->surname, node->name, node->account_number);
		node = node->next;
	}
}
...
struct Node* node_creation(char* deposit_category, char* name, char* surname, char* issuing_authority, char* time_in, char* time_expired, char* passport_series, int balance, struct Node* prev, struct Node* next)
{
...
void find_surname(struct Node* node, const char* surname)
{
	...
	do
	{
		...
		{
			printf("%s %s %lld\n", temp->surname, temp->name, temp->account_number);
		}
		temp = temp->next;
	}while(temp != NULL);
}
...
void push_back(char* deposit_category, char* name, char* surname, char* issuing_authority, char* time_in, char* time_expired, char* passport_series, int balance, struct List* list)
{
	if(list->head == NULL)
	{
		list->head = node_creation(deposit_category, name, surname, issuing_authority, time_in, time_expired, passport_series, balance, NULL, NULL);
	}
	else
	{
		...
		temp->next = node_creation(deposit_category, name, surname, issuing_authority, time_in, time_expired, passport_series, balance, temp, NULL);
	}
	list->size++;
}
...
void free_pop(struct List* list, struct Node* temp, int index)
{
	...
	if(temp->dep_size == 0)
	{
		...
		char* txt = to_string(temp->account_number);
		remove(txt);
		free(txt);
